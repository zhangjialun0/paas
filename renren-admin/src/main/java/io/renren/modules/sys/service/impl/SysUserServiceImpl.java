...
public class SysUserServiceImpl extends ServiceImpl<SysUserDao, SysUserEntity> implements SysUserService {
	...
	public void saveUser(SysUserEntity user)  {
		...
		//创建gitlabuser
		User gitlabUser =new User().withName(user.getUsername())
				.withEmail(user.getEmail())
				.withUsername(user.getUsername())
				.withSkipConfirmation(true);

		gitlabUser.setIsAdmin(false);
		gitlabUser.setCanCreateProject(true);
		gitlabUser.setCanCreateGroup(true);
		String password =user.getPassword();

			gitlabUser = gitLabApi.getUserApi().createUser(gitlabUser,password,false);
			gitLabApi.sudo(gitlabUser.getUsername());
